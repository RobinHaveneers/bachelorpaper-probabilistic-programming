...
corner_parent((X,Y),Xp,Yp) :- identifier(X,Y,I),
                            ...
                            select_weighted(I,[0,0.95,0,0.05],List,(Xp,Yp),_).

xmin_parent((X,Y),Xp,Yp) :- identifier(X,Y,I),
                            list_of_directions(List),
                            select_weighted(I,[0.475,0.05,0,0.475],List,(Xp,Yp),_).

xmax_parent((X,Y),Xp,Yp) :- identifier(X,Y,I),
                            list_of_directions(List),
                            select_weighted(I,[0.475,0,0.05,0.475],List,(Xp,Yp),_).

ymin_parent((X,Y),Xp,Yp) :- Y == 1,
                            identifier(X,Y,I),
                            list_of_directions(List),
                            select_weighted(I,[0,0.025,0.025,0.95],List,(Xp,Yp),_).

ymax_parent((X,Y),Xp,Yp) :- identifier(X,Y,I),
                            list_of_directions(List),
                            select_weighted(I,[0.95,0.025,0.025,0],List,(Xp,Yp),_).
